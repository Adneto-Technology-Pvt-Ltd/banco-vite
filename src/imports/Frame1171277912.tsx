import { AdnetoLogo } from "../assets";

function Frame1171277912Helper({ children }: React.PropsWithChildren<{}>) {
  return (
    <div className="css-ebafv2 relative shrink-0">
      <p className="adjustLetterSpacing leading-[20px] whitespace-pre text-center sm:text-left flex items-center text-[14px] text-[#e2e2e2] tracking-[0.24px]">
        {children}
      </p>
    </div>
  );
  ...
  return (
    <div className="bg-[#444444] relative w-full min-h-[60px]">
      <div className="flex flex-col sm:flex-row items-center relative w-full h-full">
        <div className="box-border content-stretch flex flex-row font-['Lato:Regular',_sans-serif] items-center justify-between leading-[0] not-italic px-4 sm:px-[60px] py-3.5 relative w-full text-[#e2e2e2] text-[12px] sm:text-[14px] text-center sm:text-left tracking-[0.24px] gap-2 sm:gap-0 h-full md:h-[60px]">

          {/* Mobile: Stack vertically, Desktop: Side by side */}
          <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-0">
            <Frame1171277912Helper>
              <span className="inline sm:hidden">© {new Date().getFullYear()} Axis Bank</span>
              <span className="hidden sm:inline">Copyright © {new Date().getFullYear()} Axis Bank</span>
            </Frame1171277912Helper>
          </div>
          ...
          <Frame1171277912Helper>
            Powered by<img src={AdnetoLogo} alt="Adneto Logo" className="h-12 w-auto"  loading="lazy" />
          </Frame1171277912Helper>
        </div>
      </div>
    </div>
